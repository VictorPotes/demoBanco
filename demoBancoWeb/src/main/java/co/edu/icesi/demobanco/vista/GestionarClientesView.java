package co.edu.icesi.demobanco.vista;
...
import co.edu.icesi.demobanco.businessdelegate.IBusinessDelegate;
import co.edu.icesi.demobanco.modelo.Clientes;
import co.edu.icesi.demobanco.modelo.TiposDocumentos;

@ManagedBean
@ViewScoped
public class GestionarClientesView {
	...
	public String action_crear() {
		...
		try {
			...
			TiposDocumentos tipoDoc = businessDelegate.findByIdTipoDocumento(idTipoDocu);
			cliente.setTiposDocumentos(tipoDoc);
			cliente.setCliActivo("S");
	...
	public void txtIdentificacionListener() {
		try {
			...
			Clientes cliente = businessDelegate.findByIdCliente(id);
			if(cliente == null) {
